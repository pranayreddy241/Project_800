...
for split in ["random", "popular", "adversarial"]:
    ...
    outfile = os.path.join(RESULTS_DIR, f"opera_pope_{split}.jsonl")

    with open(qfile, "r") as f:
        questions = [json.loads(line) for line in f if line.strip()]

    print(f"Running OPERA-lite split: {split} | questions: {len(questions)}")

    with open(outfile, "w") as fout:
        for q in tqdm(questions):
            image_path = os.path.join(IMAGE_DIR, q["image"])
            image = Image.open(image_path).convert("RGB")
            answer = ask_model(image, q["text"])
...
print("All OPERA-lite splits done.")
